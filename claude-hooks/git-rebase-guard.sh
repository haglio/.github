...
# land through the repo's merge queue. The recurring failure is an agent that hits a collision with already-landed
...
exit_block() {
  ...
  echo "Rule: when a teammate landed overlapping work, rebase onto main and resolve the conflict in place (git checkout --theirs <file> && git add; git rebase --continue / --skip past an emptied commit). Never reset-and-reapply, and never merge to sync. To squash your OWN commits use: git reset --soft \$(git merge-base main HEAD). See the law 'Sync by rebasing onto main; never git reset --soft main to tidy' in ~/.claude/engineering.md." >&2
  exit 2
}
